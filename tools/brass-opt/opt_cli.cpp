...
void print_usage(std::ostream& os, const char* prog) {
    os << "brass-opt - Brass MIR Optimizer and Compiler Tool (v" << brass::version_string() << ")\n"
       ...
       << "  --enable-osr          Run --run in a tiered program on the fast interpreter, with\n"
       << "                        On-Stack Replacement (OSR): a warm-up run, then the printed\n"
       << "                        run, which enters its hot loops' compiled OSR code\n"
