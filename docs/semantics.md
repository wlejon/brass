...
2. **IEEE-754 Strictness by Default**:
   ...
   - **NaN operands of `add`/`sub`/`mul`/`div` (`f32`, `f64`)**: when an operand is a NaN, the result is quieted (quiet bit set; sign and the rest of the payload kept). On x86 SSE (`addsd dst, src`), the destination's NaN is kept if either is NaN; on AArch64 hardware (with default-NaN off), signaling NaNs take priority over quiet NaNs, picking the first signaling NaN, or the first quiet NaN if neither is signaling. An invalid operation on non-NaN operands (`inf - inf`, `0 * inf`, `0 / 0`) gives the target's default NaN. The interpreters and the constant folder follow this through `brass/interpreter/float_arith.hpp`, since a C++ `a + b` lets the compiler commute the operands.
3. **Deterministic Integer Modulo Arithmetic**:
   ...
   - **Narrow integers (`i8`, `i16`)**: an `iN` value is an N-bit two's-complement bit pattern, and every tier computes on those N bits only:
     ...
     - *Every tier compiles every narrow operation*: tier 2 lowers `clz`, `ctz` and the overflow checks on narrow operands on x64 and AArch64 by widening the operand from N bits, like the lower tiers (`test_narrow_int_tiers.cpp`).
