...
std::vector<std::pair<const Value*, VReg>> AArch64ISel::widen_narrow_operands(const Instruction& inst,
    ...
    if (is_width_dependent(inst.opcode())) {
        ...
        throw std::logic_error("aarch64 isel: narrow " + std::string(opcode_name(inst.opcode())) +
...
bool AArch64ISel::lower_narrow_width_op(const Instruction& inst, LirBlock& lir_bb) {
    ...
    auto operand = [&](size_t i) {
        ...
        if (!r.is_valid()) {
            throw std::logic_error("aarch64 isel: narrow operand of " + std::string(opcode_name(op)) +
                                   " has no register");
        }
        return r;
    };
    ...
    switch (op) {
        case Opcode::clz: {
            ...
            append(LirOpcode::Lzcnt32, {LirOperand::vreg(dst, 4)}, {LirOperand::vreg(w, 4)});
            return true;
        }
        case Opcode::ctz: {
            ...
            append(LirOpcode::Tzcnt32, {LirOperand::vreg(dst, 4)}, {LirOperand::vreg(w, 4)});
            return true;
        }
        default:
            break;
    ...
    setcc->condition = brass::x64::Condition::NE;
