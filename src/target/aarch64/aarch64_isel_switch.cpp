...
void AArch64ISel::lower_switch(const Instruction& inst, LirBlock& lir_bb) {
        ...
        if (target.args.size() == 1) {
            ...
            if (arg_v != param_v) {
                auto mov_inst = std::make_unique<LirInst>(
                    param_v.is_xmm() ? ((psz == 16) ? LirOpcode::Movaps : (psz == 4 ? LirOpcode::Movss : LirOpcode::Movsd)) : (psz == 4 ? LirOpcode::Mov32 : LirOpcode::Mov)
                );
