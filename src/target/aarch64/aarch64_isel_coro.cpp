...
void AArch64ISel::lower_coro(const Instruction& inst, LirBlock& lir_bb) {
    ...
    switch (inst.opcode()) {
        case Opcode::coro_create: {
            ...
            // descriptor; otherwise the frame's body is @f's code here, its
            // shape computed from @f.
            if (!inst.result()) {
            ...
            if (!inst.extra_symbol().empty()) {
                ...
                mov_body->add_def(LirOperand::preg_aarch64_gpr(arg0, 8), FixedConstraint::aarch64_gpr(arg0));
        ...
        case Opcode::coro_resume: {
            ...
            if (inst.operand_count() > 2 && inst.operand(2)) {
                ...
                mov_mode->add_def(LirOperand::preg_aarch64_gpr(arg2, 4), FixedConstraint::aarch64_gpr(arg2));
                ...
                mov_zero->add_def(LirOperand::preg_aarch64_gpr(arg2, 4), FixedConstraint::aarch64_gpr(arg2));
            ...
            call_lir->add_use(LirOperand::preg_aarch64_gpr(arg2, 4), FixedConstraint::aarch64_gpr(arg2));
