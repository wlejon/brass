...
void X64ISel::lower_coro(const Instruction& inst, LirBlock& lir_bb) {
    ...
    switch (inst.opcode()) {
        case Opcode::coro_create: {
            ...
            // arguments; argument i is stored in frame slot i, where the
            // lowered body of @f loads it. With an extra symbol (a program's
            // tiered code: tier2_link.cpp) that symbol is the body's
            // descriptor, which carries the frame shape and resumes the body
            // in its best tier; otherwise the frame's body is @f's code here,
            // its shape computed from @f.
            if (!inst.result()) {
                throw_unsupported("x64 isel (coro)", "coro_create without a result");
            }
            auto call_lir = std::make_unique<LirInst>(LirOpcode::Call);
            if (!inst.extra_symbol().empty()) {
                auto mov_body = std::make_unique<LirInst>(LirOpcode::Movabs);
                mov_body->add_def(LirOperand::preg_gpr(arg0, 8), FixedConstraint::gpr(arg0));
                mov_body->add_use(LirOperand::symbol(std::string(inst.extra_symbol())));
                lir_bb.append_inst(std::move(mov_body));
                call_lir->callee_symbol = "brass_coro_create_body";
                call_lir->add_use(LirOperand::preg_gpr(arg0, 8), FixedConstraint::gpr(arg0));
                call_lir->add_use(LirOperand::symbol("brass_coro_create_body"));
            } else {
                ...
                if (!layout.fits_pointer_mask) {
                    throw_unsupported("x64 isel (coro)", "coro_create of " + std::string(inst.symbol()) +
                                      " outside a program: its reference slots do not fit the fixed-code "
                                      "frame's 64-bit mask");
                }
        ...
        case Opcode::coro_resume: {
            ...
            // The resume mode: the third operand, else 0 (Next).
            if (inst.operand_count() > 2 && inst.operand(2)) {
                VReg mode_vreg = get_vreg(inst.operand(2));
                auto mov_mode = std::make_unique<LirInst>(LirOpcode::Mov32);
                mov_mode->add_def(LirOperand::preg_gpr(arg2, 4), FixedConstraint::gpr(arg2));
                mov_mode->add_use(LirOperand::vreg(mode_vreg, 4));
                lir_bb.append_inst(std::move(mov_mode));
            } else {
                auto mov_zero = std::make_unique<LirInst>(LirOpcode::Mov32);
                mov_zero->add_def(LirOperand::preg_gpr(arg2, 4), FixedConstraint::gpr(arg2));
                mov_zero->add_use(LirOperand::imm(0, 4));
                lir_bb.append_inst(std::move(mov_zero));
            }
            ...
            call_lir->add_use(LirOperand::preg_gpr(arg2, 4), FixedConstraint::gpr(arg2));
