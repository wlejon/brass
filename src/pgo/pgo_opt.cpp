...
bool optimize_module_pgo(
    ...
    // Profile-guided inlining, then the function pipeline with its
    // profile-gated unswitching and unrolling.
    ...
    Pipeline pipeline;
    if (in_opts.enable_devirtualization) pipeline.add(passes::devirtualize());
    pipeline.add(passes::inline_calls(in_opts));
    pipeline.append(function_pipeline(l_opts));
    return run_pipeline(mod, pipeline).changed;
}
