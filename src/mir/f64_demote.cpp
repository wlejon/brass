...
static constexpr double kMaxSafeInt = 9007199254740991.0; // 2^53 - 1
static constexpr double kMinSafeInt = -9007199254740991.0;
...
bool f64_demote_pass(Function& fn, const F64DemoteOptions& options) {
    ...
    if (relevant_demote.empty()) {
        if (options.stats) {
            record_loop_stats(fn, loop_analysis, {}, options.stats);
        }
        return false;
    }
    ...
    if (options.stats) {
        record_loop_stats(fn, loop_analysis, demote_set, options.stats);
    }
        ...
        while (inst) {
            ...
            if (inst->produces_value() && demote_set.count(inst->result())) {
                ...
                if (op == Opcode::fconst_f64) {
                    ...
                    int64_t num_c = 0;
                    bool has_num_c = get_const_int_or_f64_int(inst->operand(0), num_c);

                    if (has_denom_c && denom_c == 0) {
                        // Divisor zero: prevent hardware SIGFPE trap
                        inst->set_opcode(Opcode::iconst_i64);
                        inst->set_imm_i64(0);
                        inst->operands().clear();
                        inst->set_symbol("");
                    } else if (has_denom_c && denom_c == -1 && (!has_num_c || num_c == std::numeric_limits<int64_t>::min())) {
                        // INT64_MIN % -1 overflow: prevent hardware SIGFPE trap
                        inst->set_opcode(Opcode::iconst_i64);
                        inst->set_imm_i64(0);
                        inst->operands().clear();
                        inst->set_symbol("");
                        ...
                        // Variable divisor: guard against 0 and INT64_MIN % -1
                        b.position_before(inst);
                        Value* num = inst->operand(0);
                        Value* denom = inst->operand(1);
                        Value* zero_c = b.build_iconst_i64(0);
                        Value* one_c = b.build_iconst_i64(1);
                        Value* neg_one_c = b.build_iconst_i64(-1);
                        Value* min_c = b.build_iconst_i64(std::numeric_limits<int64_t>::min());

                        Value* is_zero = b.build_eq(denom, zero_c);
                        Value* is_neg_one = b.build_eq(denom, neg_one_c);
                        Value* is_min = b.build_eq(num, min_c);
                        Value* is_overflow = b.build_and(is_neg_one, is_min);
                        Value* is_unsafe = b.build_or(is_zero, is_overflow);
                        Value* safe_denom = b.build_select(is_unsafe, one_c, denom);

                        inst->set_opcode(Opcode::smod);
                        inst->set_symbol("");
                        inst->set_operand(1, safe_denom);

                        b.position_after(inst);
                        Value* smod_res = inst->result();
                        Value* guarded_res = b.build_select(is_unsafe, zero_c, smod_res);
                        replace_all_uses(fn, smod_res, guarded_res);
                        Instruction* sel_def = guarded_res->defining_instruction();
                        if (sel_def && sel_def->operand_count() >= 3) {
                            sel_def->set_operand(2, smod_res);
                        }
                    }
